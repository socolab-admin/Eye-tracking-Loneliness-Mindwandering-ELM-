...
%% TRIMMING
%% ===============================
params.trim.start_trigger = 5;   % replace with your own trigger code
params.trim.end_trigger   = 8;  % replace with your own trigger code
params.trim.buffer = 10;
% ===============================
%% SHORT CHANNEL INDICES
%% ===============================
params.short_channels.map = [4,11,20,25,30,37,46,51]; % add short channel indices (required for now)
%params.short_channels.map = [4,9,12,16,20,23,26,29,34,39,42,46,50,53,56,59];
...
%params.post.stim.cond(1).same = true; % whether trigger start and end codes match between different conditions
%params.post.stim.cond(1).name = 'Zima';  % replace with actual condition name
%params.post.stim.cond(1).start_triggers = [1];   % add trigger code onsets
%params.post.stim.cond(1).end_triggers = [3]; % add trigger code onsets 
% make sure to have start and end triggers balanced in number of trigger
% codes etc.
%params.post.stim.cond(1).duration = 508; % replace with actual stimulus duration 

%params.post.stim.cond(2).same = true; 
%params.post.stim.cond(2).name = 'Splitscreen';  
%params.post.stim.cond(2).start_triggers = [1];  
%params.post.stim.cond(2).end_triggers = [3];
%params.post.stim.cond(2).duration = 145;

%params.post.stim.cond(3).same = true; 
%params.post.stim.cond(3).name = 'Sun';  
%params.post.stim.cond(3).start_triggers = [1];  
%params.post.stim.cond(3).end_triggers = [3];
%params.post.stim.cond(3).duration = 325; 

%params.post.stim.cond(4).same = true; 
%params.post.stim.cond(4).name = 'Turtles';  
%params.post.stim.cond(4).start_triggers = [1];  
%params.post.stim.cond(4).end_triggers = [3];
%params.post.stim.cond(4).duration = 385; 

%params.post.stim.cond(5).same = true; 
%params.post.stim.cond(5).name = 'Hexagons';  
%params.post.stim.cond(5).start_triggers = [1];  
%params.post.stim.cond(5).end_triggers = [3];
%params.post.stim.cond(5).duration = 373; 

%params.post.stim.cond(6).same = true; 
%params.post.stim.cond(6).name = 'Igloo';  
%params.post.stim.cond(6).start_triggers = [1];  
%params.post.stim.cond(6).end_triggers = [3];
%params.post.stim.cond(6).duration = 313; 

%params.post.stim.cond(7).same = true; 
%params.post.stim.cond(7).name = 'Winter';  
%params.post.stim.cond(7).start_triggers = [1];  
%params.post.stim.cond(7).end_triggers = [3];
%params.post.stim.cond(7).duration = 319; 

params.post.stim.cond(1).same = false; 
params.post.stim.cond(1).name = 'SART';  
params.post.stim.cond(1).start_triggers = [5];  
params.post.stim.cond(1).end_triggers = [8];
params.post.stim.cond(1).duration = 1; 

% Nuisance regressors
params.post.use_short = true;     % use short channels
...
params.post.epoch.hbo_hrf_delay_sec = 0; % 4.5 for specific canonical HbO peak and 6 for canonical HbR peak; 6 conservative peak for both concentrations
params.post.epoch.hbr_hrf_delay_sec = 0; 
% If saves parameters
