...
# SART version: trim Practice using trial_tbl
# ================================
...
PARSE_DIR  <- file.path(ROOT_DIR, "parsedSART")
OUT_DIR    <- file.path(ROOT_DIR, "outputs")
POST_DATA_DIR <- file.path(OUT_DIR, "pre_processedSART")
...
# ------------------------------------------------------------
# SART trim helpers: use parsed trial table, remove Practice
# ------------------------------------------------------------

get_msg_col <- function(df) {
  cn <- names(df)
  if ("msg" %in% cn) "msg"
  else if ("text" %in% cn) "text"
  else NA_character_
}

get_trial_tbl_path <- function(id_here) {
  fs::path(PARSE_DIR, paste0(id_here, "_trial_tbl.csv"))
}

read_trial_tbl <- function(id_here) {
  p <- get_trial_tbl_path(id_here)
  if (!fs::file_exists(p)) {
    stop("Missing trial table: ", p)
  }
  tt <- readr::read_csv(p, show_col_types = FALSE)

  if ("onset_time" %in% names(tt))  tt$onset_time  <- as.numeric(tt$onset_time)
  if ("offset_time" %in% names(tt)) tt$offset_time <- as.numeric(tt$offset_time)
  if ("duration_ms" %in% names(tt)) tt$duration_ms <- as.numeric(tt$duration_ms)

  tt
}

trim_raw_to_main_task <- function(raw_df, trial_tbl) {
  if (!"blockId" %in% names(trial_tbl)) {
    stop("trial_tbl does not contain blockId")
  }
  if (!"onset_time" %in% names(trial_tbl) || !"offset_time" %in% names(trial_tbl)) {
    stop("trial_tbl must contain onset_time and offset_time")
  }

  main_trials <- trial_tbl %>%
    dplyr::filter(blockId != "Practice")

  if (!nrow(main_trials)) {
    stop("No main-task trials found after removing Practice block.")
  }

  t0 <- min(main_trials$onset_time, na.rm = TRUE)
  tl <- max(main_trials$offset_time, na.rm = TRUE)

  raw_trimmed <- raw_df %>%
    dplyr::filter(time >= t0 & time <= tl) %>%
    dplyr::arrange(time)
...
# ------------------------------------------------------------
# Duration per ID: now based on main task only
# ------------------------------------------------------------

compute_one_sart <- function(path) {
  df <- readr::read_csv(
    path,
  ...
  id_here <- if ("id" %in% names(df)) unique(df$id)[1] else
    sub("_raw_with_id_msg$", "", fs::path_ext_remove(fs::path_file(path)))

  # keep stimulus only if present; no video mapping
  stimulus <- if ("stimulus" %in% names(df)) {
    (unique(na.omit(df$stimulus)) %>% as.character())[1]
  } else {
    NA_character_
  }

  trial_tbl <- read_trial_tbl(id_here)
  trimmed <- trim_raw_to_main_task(df, trial_tbl)

  duration_ms <- trimmed$tl - trimmed$t0

  tibble::tibble(
    id = id_here,
    stimulus = stimulus,
    onset_ms = trimmed$t0,
    last_frame_ms = trimmed$tl,
    duration_ms = duration_ms,
    duration_s = round(duration_ms / 1000, 3),
    n_main_trials = nrow(trimmed$main_trials),
    n_practice_trials = sum(trial_tbl$blockId == "Practice", na.rm = TRUE)
  )
}

res_one <- compute_one_sart(raw_path)

combined_file <- file.path(OUT_DIR, "duration_per_id.csv")
first_write   <- !file.exists(combined_file)
...
cat("Appended duration for", id, "to:", combined_file, "\n")
print(res_one, n = nrow(res_one))

# ============================================================
# Blink & Off-Screen mapping for ONE id (the array task's id)
# -> Append ONE row per task into outputs/raw_pre_summary.csv
# ============================================================
...
# Trim to MAIN task only using trial_tbl
trial_tbl <- read_trial_tbl(id)
trimmed <- trim_raw_to_main_task(raw, trial_tbl)
raw <- trimmed$raw_trimmed
main_trials <- trimmed$main_trials

cat(sprintf(
  "Trimmed %s to main task only: %.0f to %.0f ms | kept %d main trials\n",
  id, trimmed$t0, trimmed$tl, nrow(main_trials)
))
...
# 3) Pupil validity per sample
if (!all(c("psl","psr") %in% names(raw))) {
  stop("For id ", id, ": columns 'psl' and 'psr' not found in raw file.")
}
invalid_L <- is.na(raw$psl) | (raw$psl <= 0) | !is.finite(raw$psl)
invalid_R <- is.na(raw$psr) | (raw$psr <= 0) | !is.finite(raw$psr)


# 4) Missed vs false blinks (per-eye)
missed_L <- sum(invalid_L & !blink_L, na.rm = TRUE)
missed_R <- sum(invalid_R & !blink_R, na.rm = TRUE)
valid_L  <- !invalid_L
valid_R  <- !invalid_R
...
# 7) Screen fixation validity per sample (step1-like)
screen_x_min <- 0
screen_x_max <- 1919
screen_y_min <- 0
screen_y_max <- 1079
...
# Per-axis off-screen flags, for reporting only
...
# 11) Append THIS row to one combined CSV, then decide on dropout
...
# 1) Add relative time for THIS id
t0 <- df$time[which.min(df$time)]
df$t_rel_ms <- as.numeric(df$time) - as.numeric(t0)
...
resample_1khz <- function(df, id, blink_cols = c("blink_L","blink_R")) {
  ...
  grid <- seq(ceiling(t0), floor(t1), by = 1)

  step_cols <- intersect(blink_cols, names(df))
  num_cols  <- names(df)[vapply(df, is.numeric, logical(1))]
  num_cols  <- setdiff(num_cols, c("time", step_cols))

  out <- data.frame(time = grid, stringsAsFactors = FALSE)

  for (nm in num_cols) {
    v <- df[[nm]]
    ok <- is.finite(df$time) & is.finite(v)
...
# 3) Resample THIS id if needed
hz <- estimate_hz(df$time)
if (is.finite(hz) && abs(hz - 1000) <= 1) {
  ...
  df_rs          <- resample_1khz(df, id)
  cat(sprintf("ID %s: resampled to 1000 Hz (was %.3f Hz)\n", id, hz))
}

# 4) Put back to raws_pre for downstream steps
raws_pre[[id]] <- df_rs
...
# ------------------------------------------------------------
# Masking (ONE id) — match step1 behavior
#   * only pad blinks (±50 ms) using expand_mask on blink flags
#   * do NOT pad or use off-screen flags for interpolation
#   * compute invalid_* flags for ps/x/y
#   * interp_ps* = pad_blink_eye OR invalid_ps*
#   * interp_x*/y* = pad_blink_eye OR invalid_x*/y*
# ------------------------------------------------------------

expand_mask <- function(t, mask, pad_ms = 50L) {
...
# blink pads only (±50 ms)
pad_blink_L <- expand_mask(df[[tcol]], df$blink_L, 50L)
pad_blink_R <- expand_mask(df[[tcol]], df$blink_R, 50L)
...
# ---------- Build per-variable interpolate booleans ----------
...
# Gaze: no blink padding, only invalid gaze values
...
cat("Masking complete for id ", id,
    ": created interp_* booleans and set those samples to NA (blink±50ms for pupil; invalid samples only for gaze). \n", sep = "")

# ================================
# Per-ID interpolation summary CSV (append to one combined file)
# ================================
...
# ===========================
# Interpolation for ONE id
# ===========================

fill_only_where <- function(x, t, mask) {
  if (!length(x) || !length(mask) || !any(mask)) return(x)
  x_work <- x
  x_work[mask] <- NA
  x_lin <- tryCatch(zoo::na.approx(x_work, x = t, na.rm = FALSE),
                    error = function(e) x_work)
  x_out <- x
...
fp <- file.path(POST_DATA_DIR, paste0("data_preprocessedSART_", safe(id), ".csv"))
